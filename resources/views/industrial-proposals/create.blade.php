            ...
            <div class="flex gap-20 justify-between">
                            ...
                            <select id="supervisor_id" name="supervisor_id" class="input select2">
                                ...
                                @foreach ($supervisors as $supervisor)
                                    <option value="{{ $supervisor->id }}"
                                        {{ old('supervisor_id', $industrialProposal->supervisor_id ?? '') == $supervisor->id ? 'selected' : '' }}>
                                        {{ $supervisor->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="flex-1">
                    ...
                    <div id="company_manage">
                        <div class="mb-4">
                            ...
                            <input type="text" id="company" name="company" class="input"
                                ...
                                value="{{ old('company', $industrialProposal->company ?? '') }}" @readonly($industrialProposal)>
                            @error('company')
                                <p class=" validate_error">{{ $message }}</p>
                            @enderror
                        </div>
                                ...
                                <select id="company_id" name="company_id" class="input select2">
                                    ...
                                    @foreach ($companies as $company)
                                        <option value="{{ $company->id }}"
                                            {{ old('company_id', $industrialProposal->company_id ?? '') == $company->id ? 'selected' : '' }}>
                                            {{ $company->name }} - ({{ $company->available_quantity }}) </option>
                                    @endforeach
                                </select>
                                @error('company_id')
                                    <p class="validate_error">{{ $message }}</p>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="industrial_supervisor_name" name="industrial_supervisor_name"
                            class="input" placeholder="Enter your industrial supervisor name"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="industrial_supervisor_phone" name="industrial_supervisor_phone"
                            class="input" placeholder="Enter your industrial supervisor phone"
